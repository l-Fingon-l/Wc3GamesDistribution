...
class File(object):
    ...
    def _read_blocks(self):
        ...
        # Fingon's optimizations for replay winner search

        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

        # read the first block for metadata
        ...
        # skip all the blocks before the last 2
        # they are all of the same size

        for n in range(self.nblocks - 3):
            block_size = b2i(f.read(WORD))
            self.loc += 10 + block_size

        # ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

        # read the last 2 blocks
    ...
    def _parse_blocks(self, data):
        ...
        while blockid != 0:
            ...
            while True:
                if blockid == 0x17:
                    if b2i(data[(pos + 1):(pos + 1 + DWORD)]) in _reasons:
                        break
                pos += 1
                blockid = data[pos]
            data = data[pos:]
    ...
    '''
        while blockid != 0:
            pos = 0
            while True:
                if blockid not in _parsers:
                    pos += 1
                    blockid = b2i(data[pos])
                elif b2i(data[(pos+1):(pos+1+DWORD)]) in _reasons:
                    break
            data = data[pos:]
            offset = _parsers[blockid](data)
            data = data[offset:]
    ...
    def _parse_startup(self, data):
        ...
        if b2i(data[offset]) != 0x19:
            ...
            offset += 12
            if self.player_count != 24:  # pre-flo
                self.flo = bool(False)
                int_attempts = 0
                ...
                while (True):
                    if b2i(data[offset]) == 0x19:
                        if b2i(data[offset + 1]) == 0xdf: break
                    offset += 1
